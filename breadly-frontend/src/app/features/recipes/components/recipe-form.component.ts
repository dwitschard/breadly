...
@Component({
  ...
  template: `
    <form (ngSubmit)="onSubmit()" class="flex gap-3">
      <label for="recipe-name" class="sr-only">Rezeptname</label>
      <input
        id="recipe-name"
        type="text"
        [(ngModel)]="name"
      ...
      <button
        ...
        class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
      >
        Hinzufuegen
      </button>
    </form>
