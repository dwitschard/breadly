...
// ── BakingTimeline ───────────────────────────────────────────
function BakingTimeline({ steps, startTime }) {
  const [hovered, setHovered] = useState(null);
  const totalMins = steps.reduce((s, st) => s + st.mins, 0);

  // Pre-compute start minute offset for each step
  const offsets = steps.reduce((acc, st, i) => {
    acc.push(i === 0 ? 0 : acc[i-1] + steps[i-1].mins);
    return acc;
  }, []);

  const fmtTime = (base, offsetMins, durationMins) => {
    const start = new Date(base.getTime() + offsetMins * 60000);
    const end   = new Date(base.getTime() + (offsetMins + durationMins) * 60000);
    const p = d => d.toLocaleTimeString('de-DE', { hour:'2-digit', minute:'2-digit' });
    ...
    <div style={{ width:'100%' }}>
      ...
      <div style={{ position:'relative', display:'flex', marginTop:6 }}>
        {steps.map((st, i) => {
          ...
          const showWindow = startTime && st.type === 'active';
          return (
            <div key={i} style={{
              ...
              {showWindow && (
                <span style={{
                  ...
                  color: T.amber700, fontWeight:500,
                  whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis',
                  marginTop:1,
                }}>
                  {fmtTime(startTime, offsets[i], st.mins)}
                </span>
              )}
            </div>
          );
